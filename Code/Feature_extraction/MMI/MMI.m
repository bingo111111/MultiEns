...
protein=regexprep(protein,'A|G|V','1');%字符串查找并替换
...
protein_number_index=1:1:7;%即[1, 2, 3, 4, 5, 6, 7]
...
for i=1:groups_no
	num_a = length(regexp(protein,protein_groupings_index(i)));%用于对字符串进行查找，大小写敏感
    CD(i)= (num_a+1)/(L+1);
	F_A(i)= num_a/L;
end
...
for i = 1:n_MMI
	His_abc = [];
	ll_abc = abc(i,:);
	h0=length(find(ll_abc==1));%返回将符合条件的索引添加到数组中
...
feature(find(isnan(feature)))=0;%将空值赋值为0
feature(find(isinf(feature)))=0;%将无穷赋值为0